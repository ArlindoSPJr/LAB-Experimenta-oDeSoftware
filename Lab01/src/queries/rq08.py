...
CAMINHO_CSV_AMOSTRA = Path(__file__).resolve().parent.parent.parent / "data" / "amostras" / "rq08_amostra.csv"
...
def montar_query_busca() -> str:
    ...
    Usa `isArchived`, exposto direto pela API, para verificar se
    repositórios populares são formalmente arquivados/descontinuados.
    """
    return """
...
def coletar_amostra(quantidade: int | None = None, token: str | None = None) -> list[dict]:
    """Coleta a amostra de repositórios com estrelas e status de arquivamento."""
        ...
        linhas.append(
            {
                ...
                "arquivado": repositorio["isArchived"],
            }
        )
    return linhas
...
def salvar_csv(linhas: list[dict], caminho: Path) -> None:
    ...
    with caminho.open("w", newline="", encoding="utf-8") as arquivo:
        escritor = csv.DictWriter(arquivo, fieldnames=["repositorio", "estrelas", "arquivado"])
