...
CAMINHO_CSV_AMOSTRA = Path(__file__).resolve().parent.parent.parent / "data" / "amostras" / "rq03_amostra.csv"
...
def montar_query_busca() -> str:
    ...
    Usa `releases { totalCount }` para obter o total de releases lançadas.
    """
    return """
...
def coletar_amostra(quantidade: int | None = None, token: str | None = None) -> list[dict]:
    """Coleta a amostra de repositórios e o total de releases de cada um."""
        ...
        linhas.append(
            {
                ...
                "total_releases": repositorio["releases"]["totalCount"],
            }
        )
    return linhas
...
def salvar_csv(linhas: list[dict], caminho: Path) -> None:
    ...
    with caminho.open("w", newline="", encoding="utf-8") as arquivo:
        escritor = csv.DictWriter(arquivo, fieldnames=["repositorio", "estrelas", "total_releases"])
