...
CAMINHO_CSV_AMOSTRA = Path(__file__).resolve().parent.parent.parent / "data" / "amostras" / "rq11_amostra.csv"
...
def montar_query_busca() -> str:
    ...
    Usa `fundingLinks`, exposto direto pela API, para verificar se
    repositórios populares recebem apoio financeiro direto (GitHub Sponsors
    e outras plataformas de financiamento configuradas no `FUNDING.yml`).
    """
    return """
...
def coletar_amostra(quantidade: int | None = None, token: str | None = None) -> list[dict]:
    """Coleta a amostra de repositórios com estrelas e status de financiamento configurado."""
    ...
    for repositorio in dados["search"]["nodes"]:
        plataformas = [link["platform"] for link in repositorio["fundingLinks"]]
        linhas.append(
            {
                "repositorio": f"{repositorio['owner']['login']}/{repositorio['name']}",
                "estrelas": repositorio["stargazerCount"],
                "possui_funding": bool(plataformas),
                "plataformas_funding": ";".join(plataformas) if plataformas else "N/A",
            }
        )
    return linhas
...
def salvar_csv(linhas: list[dict], caminho: Path) -> None:
    ...
    with caminho.open("w", newline="", encoding="utf-8") as arquivo:
        escritor = csv.DictWriter(
            arquivo, fieldnames=["repositorio", "estrelas", "possui_funding", "plataformas_funding"]
        )
