...
CAMINHO_CSV_AMOSTRA = Path(__file__).resolve().parent.parent.parent / "data" / "amostras" / "rq_bonus_licenca_amostra.csv"
...
def montar_query_busca() -> str:
    ...
    Usa `licenseInfo.name`, exposto direto pela API, para verificar se
    repositórios populares tendem a adotar licenças permissivas.
    """
    return """
...
def coletar_amostra(quantidade: int | None = None, token: str | None = None) -> list[dict]:
    """Coleta a amostra de repositórios com estrelas e licença."""
    ...
    for repositorio in dados["search"]["nodes"]:
        licenca = repositorio["licenseInfo"]
        linhas.append(
            {
...
def salvar_csv(linhas: list[dict], caminho: Path) -> None:
    ...
    with caminho.open("w", newline="", encoding="utf-8") as arquivo:
        escritor = csv.DictWriter(arquivo, fieldnames=["repositorio", "estrelas", "licenca"])
